...
class _DetalleRestaurantePageState extends State<DetalleRestaurantePage> {
  ...
  Future<void> cargarComentarios() async {
    final lista = await DBService.instance.obtenerComentariosDeRestaurante(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (nuevoTexto.isNotEmpty) {
                await DBService.instance.editarComentario(
                  comentarioId,
                  nuevoTexto,
                );
                await cargarComentarios();
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          backgroundColor: esFavorito ? Colors.green : Colors.orange,
        ),
      );
    } catch (e) {
  ...
  Widget _buildImagen(String imagePath) {
    if (imagePath.isEmpty) return _fallbackImage();

    final esUrl = imagePath.startsWith('http');
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // =============== TÍTULO DEL RESTAURANTE + FAVORITO ===============
              Row(
