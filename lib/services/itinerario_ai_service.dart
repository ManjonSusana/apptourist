...
class ItinerarioAIService {
  ...
  // --------------------------------------------------
  // Construye un catálogo compacto basado en tu BD
  // --------------------------------------------------
  Future<Map<String, dynamic>> _construirContextoCatalogo(
      DateTime fechaInicio, int dias) async {
    final fechaFin = fechaInicio.add(Duration(days: dias - 1));

    final lugares = await _db.obtenerLugares();
    final restaurantes = await _db.obtenerRestaurantes();
    final bares = await _db.obtenerBares();
    final hitos = await _db.obtenerFechasDestacadas();
    ...
    for (final h in hitos) {
      ...
      if (intersectaRango || permanente) {
        ...
        eventosPorHito[hitoId] = await _db.obtenerEventosPorHito(hitoId);
      }
    }
    ...
    return {
      ...
      'bares': bares
          ...
          .toList(),
      'fechas_destacadas': hitosEnRango
          .map((h) {
            final int hitoId = h['id'] as int;
